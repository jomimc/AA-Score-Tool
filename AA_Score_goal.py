...
def calc_vdw_descriptor(protein, interactions):
    ...
    vdw_energy = np.sum(np.power((radii_sum / dist), 8) - 2 * np.power((radii_sum / dist_matrix), 4))
    return vdw_energy


def calc_ele_descriptor(result, mol_lig, mol_prot):
    # Indices (i,j) should be stored in interactions (PLInteraction) object
    idx = interactions.ele_idx
    # Partial charges (np.ndarray) should be stored in protein object
    partial_charge = protein.partial_charge[idx]
    # Multiply the charges (q[i] * q[j])
    charge_product = partial_charge.product(axis=1)
    # Get the interatomic distances (dist_mat) from interactions object
    dist = interactions.dist_mat[idx[:,0], idx[:,1]]
...
def calc_score(mol_prot):
    ...
    hbond_energy = calc_hbonds_descriptor(mol_prot, interactions)
    hphob_energy = calc_hydrophybic_descriptor(mol_prot, interactions)
    vdw_energy = calc_vdw_descriptor(mol_prot, interactions)
    ele_energy = calc_ele_descriptor(mol_prot, interactions)
